...
- **黒画面モード**: タイマー開始5秒後に画面が真っ黒になり、バッテリーを節約
  - タップすると通常画面に戻ります
  - 黒画面中もタイマーは裏側で継続してカウント
  - Wake Lockは有効なまま（スマホの電源はON状態）

### 2. 自動停止機能
アプリからフォーカスが外れた場合(ホーム画面に戻る、別アプリを起動など)、タイマーを自動的に停止します。

**動作詳細:**
- ✅ **継続**: アプリ画面表示中(Wake Lock有効)
- ✅ **継続**: 黒画面モード中(Wake Lock有効、画面は真っ黒)
- ❌ **停止**: 電源ボタンで画面をオフにした時
- ❌ **停止**: ホーム画面に戻った時
- ❌ **停止**: 別のアプリを起動した時
- ❌ **停止**: アプリを完全終了した時
...
**黒画面モードの動作:**
1. Startボタンを押すとタイマー開始
2. 5秒後に画面が真っ黒になる
3. タイマーは裏側で継続（Wake Lock有効）
4. 画面をタップすると通常表示に戻る
5. 再度5秒経過すると黒画面に戻る
...
[5秒経過]
  ↓
[TimerProvider] 黒画面モードフラグをON
  ↓
[UI] 画面全体に黒いオーバーレイを表示
  ↓
[ユーザータップ]
  ↓
[TimerProvider] 黒画面モードフラグをOFF
  ↓
[UI] 通常表示に戻る（タイマーは継続中）
```
...
#### 3. アプリフォーカス喪失
```
[AppLifecycleObserver] paused/inactive/hidden状態を検知
  ↓
...
**注意:**
- 黒画面モード中は、アプリはフォアグラウンドで動作しているため、タイマーは継続します
- 電源ボタンで画面をオフにすると、アプリがバックグラウンドに移行し、タイマーが停止します
...
```
[停止中] ──Start──> [実行中(通常表示)] ──Stop──> [停止中]
                        │
                        │ (5秒後)
                        ↓
                [実行中(黒画面モード)] ←──Tap──→ [実行中(通常表示)]
                        │
                        │ (電源ボタン/ホーム)
                        ↓
                [バックグラウンド移行]
...
#### 黒画面モードの仕様
- **自動表示タイミング**: タイマー開始から5秒後
- **解除方法**: 画面をタップ
- **再表示**: 通常表示に戻ってから5秒後に再び黒画面モードへ
- **表示内容**: 真っ黒な画面 + 小さなヒントテキスト「タップして画面を表示」
- **バッテリー節約効果**: OLED/AMOLEDディスプレイでは黒色表示時に消費電力が少ない

#### 自動停止トリガー
- `AppLifecycleState.paused`: アプリがバックグラウンドに移行(ホームボタン、別アプリへの切り替え)
- `AppLifecycleState.inactive`: アプリが非アクティブ(通知センター開くなど)
- `AppLifecycleState.hidden`: 電源ボタンで画面をオフにした時
- `AppLifecycleState.detached`: アプリが終了

**重要:**
- **黒画面モード中はタイマー継続**: アプリはフォアグラウンドで動作し続けるため、タイマーは停止しません
- **電源ボタンでは停止**: 電源ボタンを押すとアプリがバックグラウンドに移行し、タイマーが停止します
...
黒画面モードは、タイマー開始から5秒後に自動的に表示されます。表示されない場合:
1. タイマーが正しく開始されているか確認
2. 5秒間待機してみる
3. アプリを再起動してみる

### 黒画面モードを解除できない

画面をタップすると通常表示に戻ります。タップしても反応がない場合:
1. 画面の中央部分をタップしてみる
2. アプリを再起動してみる
